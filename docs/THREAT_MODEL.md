...
| **ERC-4626 yield vault** | Receives `deposit`, honors `redeem`. On mainnet this is `SimulatedYieldVault` (simulated 3% APY). | N/A. | **Semi-trusted / possibly hostile.** The hook is written so a reverting or lossy vault cannot break solvency or strand other users (see §3.1 vault path, §4 vault-redeem-revert). |
...
### 2.7 Informational telemetry — `afterAddLiquidity` / `lpPositions` / `getLPPosition` (J1, accepted)
`_afterAddLiquidity` records an `LPPosition` keyed by an LP address **decoded from caller-supplied `hookData`**. This is spoofable (an attacker can write any address's entry). It is **dead telemetry**: `lpPositions` is never read by any payout, access-control, eligibility, or accounting path. See §5 (Accepted Risks, J1).
...
- **Event cast is saturating.** `OrderFilled.executionPrice` uses `_executionPrice` (saturating), so an extreme-price pool cannot revert the fill path; accounting keeps the reverting `sqrtPriceToUint128` only where a revert would fail-closed.
...
1. **J1 — spoofable `lpPositions` telemetry.** `afterAddLiquidity` writes an `LPPosition` keyed by an LP address decoded from caller-controlled `hookData`, so anyone can forge entries. **Accepted because it is dead telemetry:** `lpPositions`/`getLPPosition` are never read by any payout, access-control, eligibility, or accounting path. Every payout gates on `ownerOf` + the `min(yield, IL)` + `redeemed` caps. The cost of removing it (bytecode churn, re-test, re-audit) outweighs the value; it is flagged for a later cleanup pass, not a fix.

2. **No `MIN_ORDER_SIZE`.** A single global minimum cannot serve both 6-decimal USDC (the live Unichain `currency0`) and 18-decimal tokens — e.g. `1e15` would impose a ~$1B floor on USDC orders (a decimals footgun). Anti-dust griefing is already mitigated by M3 (O(1) removal) + the per-swap gas budget + the real `transferFrom`/`_mint` cost of each order. If added later it must be per-pool / decimal-aware.

3. **Partial fills close-and-refund (no re-queue).** A partially filled order is marked filled and refunds the unconsumed input rather than re-resting the remainder. Re-queuing would overload the dual-purpose `amount0/amount1` fields (input custody vs. output) and risk the M2 solvency invariant. The fill is still at trigger-or-better within tolerance; the user simply re-submits the remainder if desired.

4. **`netAmount.toUint96()` can revert on an unreachable single-order output.** A single order producing `> ~7.9e28` wei of output would revert the `toUint96` cast in `_executeOrder`. This is **fail-closed** (no silent loss) and unreachable for any realistic pair/size (`uint96` output ≈ 7.9e10 tokens at 18 decimals). Not introduced by any fix; left as-is because it cannot cause loss.

5. **`sqrtPriceToUint128` can panic for an extreme pool price (> ~1.85e19).** The `sqrtPrice²` step in the reverting price helper (used in `_afterSwap` eligibility) can panic for a pool priced above ~1.85e19. This is a **self-DoS confined to the griefer's own hooked pool** — H1/H2 isolation means it cannot affect any other pool's swaps or orders. Accepted as an extreme-pool-only, self-inflicted condition.

6. **M4 `_tolerantSqrtLimit` overflow clamp is dead code.** After the L1 `MAX_TRIGGER_PRICE = 1e36` bound, the `priceX192 > type(uint256).max >> 96` clamp branch in `_tolerantSqrtLimit` is provably unreachable for every creatable order (the tolerant price stays ~18× below the overflow point). It is retained as defense-in-depth, not relied upon.

7. **IL is a bounded heuristic, not precise IL (J2 / M1 residual).** `_calculateIL` is a 2nd-order Taylor approximation of the V2 IL curve scaled by output notional, referenced against the never-updated pool-init baseline, valid only for <50% moves (underestimates beyond). Its **accuracy** is a disclosed UX limitation, not a safety issue: the payout is hard-capped at the user's realized vault yield and at `redeemed`, so imprecision or manipulation can only change the *size* of a self-funded rebate, never cause fund loss. A full oracle was judged disproportionate for a yield-capped rebate.

8. **Admin trust surface.** The `Ownable` owner can set fees (≤ 50 BPS), withdraw accrued fees, and force-cancel **unfilled** orders (funds always returned to the current NFT owner). The owner **cannot** touch filled-order output, exceed `MAX_FEE_BPS`, withdraw beyond `pendingFees`, or seize a vault-backed position. This bounded admin power is the intended trust model for the current single-EOA deployment; a production deployment should move ownership to a timelock/multisig.
